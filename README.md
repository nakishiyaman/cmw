# Claude Multi-Worker Framework (cmw) v0.5.2
...
**注記:** v0.5.2では、Claude Code Pluginとしてのインストールはまだ対応していません。現在はCLIツールとしてPyPI経由でインストールしてください。Plugin対応はv0.6.0で実装予定です。
...
### 4. 🔒 100%型安全 + クリーンコード（v0.5.2 NEW!）
**mypy完全対応**で型エラーゼロを達成。IDE補完が完璧に機能し、リファクタリングも安心。**ruff format**で統一されたコードスタイル、**90%テストカバレッジ**（399テスト）で高品質を保証。CI/CDで型チェックとlintを自動化し、品質の高いコードベースを維持します。
