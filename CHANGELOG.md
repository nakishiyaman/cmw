...
### Fixed
- **循環依存の自動修正が機能しない重大バグを修正**
  - **根本原因**: `detect_cycles` がエッジ情報（依存の向き）を喪失していた
  - **影響**: 9件の依存関係を削除しても10件の循環が残る問題
  - **解決**: エッジ情報を保持することで、正しいエッジが削除され循環が確実に解消される
  - 従来: ノードリスト `['A', 'B', 'C']` → エッジを推測（不正確）
...
- **循環依存の検証結果が誤って表示される問題を修正**
  - サマリーテーブルが修正前の循環数を表示していた
  - 修正が保存されないケースがあった（循環が残る場合）
  - 修正結果の詳細表示を追加（削除数、修正前後の循環数）

- **コードベース全体でエッジ/ノード混同バグを監査・修正**
  - `graph_visualizer.py` の循環表示を改善
  - コードの意図を明確にし、将来のバグを防止
  - 全NetworkX使用箇所をレビュー完了
...
### Changed
- **循環依存検出のAPI変更（内部）**
  - `detect_cycles()` の戻り値: `List[List[str]]` → `List[List[Tuple[str, str]]]`
  - より正確な依存関係情報を提供
